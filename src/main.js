import './style.css';
import {createIcons,House,Sun,Moon,Lightbulb,Snowflake,Tv,Refrigerator,WashingMachine,Wind,Flame,Speaker,X,Plus,Minus,RotateCcw,Layers,Maximize,ChevronRight,ArrowUpRight,Armchair,BedDouble,CookingPot,Bath,Package,Flower2,Footprints,Check,ChevronDown,SlidersHorizontal} from 'lucide';
import {createHome} from './model.js';
import {rooms,createDevices,applyScene,power,restore} from './state.js';

const icons={House,Sun,Moon,Lightbulb,Snowflake,Tv,Refrigerator,WashingMachine,Wind,Flame,Speaker,X,Plus,Minus,RotateCcw,Layers,Maximize,ChevronRight,ArrowUpRight,Armchair,BedDouble,CookingPot,Bath,Package,Flower2,Footprints,Check,ChevronDown,SlidersHorizontal};
...
let room='all',selected=null,night=false,walls='auto',filter='all',sceneName='',lastFocus=null;
...
$('#app').innerHTML=`
 ...
 <div class="view-tools"><button id="view-3d" class="active" title="三维视角">3D</button><button id="view-top" title="俯视户型">2D</button><span></span><button id="zoom-in" aria-label="放大">${icon('plus')}</button><button id="zoom-out" aria-label="缩小">${icon('minus')}</button><button id="reset-view" aria-label="重置视角">${icon('rotate-ccw')}</button></div>
 ...
 <aside id="detail" class="detail-panel" aria-label="设备控制" hidden></aside><div id="toast" role="status"></div>
...
let home;
try{home=createHome($('#canvas'),devices,selectDevice);}catch(error){console.error(error);$('#canvas').innerHTML='<div class="webgl-error"><h2>当前无法显示三维空间</h2><p>请启用浏览器硬件加速或使用支持 WebGL 2 的浏览器。设备列表仍可使用。</p></div>';}
...
document.addEventListener('keydown',e=>{if(e.key==='Escape'){closeDetail();document.body.classList.remove('devices-open');}});
renderRooms();renderDevices();refreshIcons();
